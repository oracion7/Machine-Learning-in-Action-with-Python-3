...
def pearsSim(inA, inB):
    if len(inA) < 3:
        return 1.0
    return 0.5 + 0.5 * corrcoef(inA, inB, rowvar=0)[0][1]  # 从-1~+1归一化到0~1


def cosSim(inA, inB):
    num = float(inA.T * inB)
    denom = la.norm(inA) * la.norm(inB)
    return 0.5 + 0.5 * (num / denom)  # 从-1~+1归一化到0~1
...
def svdEst(dataMat, user, simMeas, item):
    ...
    for j in range(n):
        ...
        print("the {0} and {1} similarity is: {2:f}".format(item, j, similarity))
        simTotal += similarity
        ratSimTotal += similarity * userRating
    if simTotal == 0:
        return 0
