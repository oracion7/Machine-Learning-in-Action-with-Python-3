    ...
    for i in range(n):
        ...
        for j in range(-1, int(numSteps) + 1):
            for inequal in ['lt', 'gt']:
                ...
                #     thresh {1:.2f}, \
                #     thresh inequal: {2:s}, \
                #     the weighted error is {3:.3f}".format(i, threshVal, inequal, float(weightedError)))
...
def adaBoostTrainDS(dataArr, classLabels, numIt=40):
    ...
    for i in range(numIt):
        ...
        # print("aggClassEst: {0}".format(aggClassEst))
        aggErrors = multiply(sign(aggClassEst) != mat(classLabels).T, ones((m, 1)))  # 错误率累加计算
